...
x_c, y_c = get_points_on_circle(.275, .6, .6, ds)
...
coord_u = calc_q(X_u.flatten(order='F'), Y_u.flatten(order='F'), x_c, y_c, ds)
coord_v = calc_q(X_v.flatten(order='F'), Y_v.flatten(order='F'), x_c, y_c, ds)
...
X_u_flat = X_u.flatten(order='F')
Y_u_flat = Y_u.flatten(order='F')
X_v_flat = X_v.flatten(order='F')
Y_v_flat = Y_v.flatten(order='F')
...
H_u = HF(F_x, coord_u, ds, (nx-1)*ny)
H_v = HF(F_y, coord_v, ds, (ny-1)*nx)

H = np.append(H_u, H_v)
h_x, h_y = unpack_q(H, nx, ny)
...
plt.figure()
plt.contourf(X_v, Y_v, [row[1:] for row in h_y])
plt.title('H_v')
plt.xlim(0,x_max)
plt.ylim(0,y_max)
plt.colorbar()
